    ...
    >
        ...
        >
            <div v-if="!collapsed" class="overflow-hidden whitespace-nowrap">
                <h1 class="font-bold text-lg uppercase tracking-widest text-green-400">
                    Admin SM
                </h1>
            </div>
            ...
            >
                <i :class="[collapsed ? 'fas fa-angle-double-right' : 'fas fa-angle-double-left', 'text-2xl transition-colors duration-300 text-gray-400 hover:text-red-500']"></i>
            </button>
        </div>
        ...
        <nav class="flex-1 mt-4 space-y-1 px-2 overflow-y-auto">
            ...
            >
                ...
                <transition name="fade">
                    <span v-if="!collapsed" class="ml-4 flex-1 whitespace-nowrap">
                        {{ item.label }}
                    </span>
                </transition>
            </Link>
        ...
        <div class="p-4 border-t border-gray-800 text-center text-gray-500 text-xs">
...
import { Link } from "@inertiajs/vue3"; // Ensure this is imported
...
const menuItems = reactive([
    ...
    { label: "Teacher",   icon: "fas fa-chalkboard-teacher", route: "/admin/teacher" },
